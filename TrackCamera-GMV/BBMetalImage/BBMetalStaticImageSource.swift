    ...
    var bb_cgimage: CGImage? {
        ...
        if let cfdata = CFDataCreate(kCFAllocatorDefault, &data, bytesPerRow * height),
            ...
            let cgimage = CGImage(width: width, height: height, bitsPerComponent: 8, bitsPerPixel: 32,
                                  bytesPerRow: bytesPerRow, space: BBMetalDevice.sharedColorSpace,
                                  bitmapInfo: CGBitmapInfo(rawValue: bitmapInfo), provider: dataProvider, decode:nil, shouldInterpolate: true, intent: .defaultIntent){
            return cgimage
        }
        return nil
    }
