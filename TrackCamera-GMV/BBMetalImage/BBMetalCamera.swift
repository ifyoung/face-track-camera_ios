...
public class BBMetalCamera: NSObject {
    ...
    public init?(sessionPreset: AVCaptureSession.Preset = .high, position: AVCaptureDevice.Position = .front) {
        _consumers = []
        _canTakePhoto = false
        _isPaused = false
        _benchmark = false
...
//        connection.videoOrientation = .portrait
    ...
    public func takePhoto(_ withSettings: AVCapturePhotoSettings? = nil,flashState:AVCaptureDevice.FlashMode = .off) {
        lock.wait()
        if let output = photoOutput,
            _photoDelegate != nil {
            let currentSettings = withSettings ?? AVCapturePhotoSettings(format: [kCVPixelBufferPixelFormatTypeKey as String : kCVPixelFormatType_32BGRA])
            currentSettings.flashMode = flashState
...
//        delegate.cameraVideo(self, didOutput: sampleBuffer)//自动闪光灯判断数据提取
        guard let delegate = videoDelegate else { return }
               delegate.cameraVideo(self, didOutput: sampleBuffer)//自动闪光灯判断数据提取
    }
