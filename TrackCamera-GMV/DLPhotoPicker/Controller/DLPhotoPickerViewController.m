...
//         photoCollectionViewController.delegate = self;
         if([self.photoCollections count] > 1){
             //0->最近项目，所有的；1->最近添加，最新入库的那些
             if([self.photoCollections[1] count] == 0){
                 photoCollectionViewController.photoCollection = self.photoCollections[0];
             }else{
                 photoCollectionViewController.photoCollection = self.photoCollections[1];
             }
         }else{
             photoCollectionViewController.photoCollection = self.photoCollections[0];
         }
        ...
        [self pushViewController:photoCollectionViewController animated:NO];
