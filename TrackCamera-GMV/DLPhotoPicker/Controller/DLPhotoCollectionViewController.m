...
//    if(self.picker.isReturnFromAlbum){
//        [self updateNavigationTitle];
...
//    self.picker.isReturnFromAlbum = NO;
}
...
- (void)resetAssetsAndReload
{
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, 0.1 * NSEC_PER_SEC), dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        weakSelf.assets = [[[DLPhotoManager sharedInstance] assetsForPhotoCollection:weakSelf.photoCollection] mutableCopy];
...
#pragma mark - 相册列表
- (void)createAlbumButton{
    self.navigationItem.rightBarButtonItem =  [[UIBarButtonItem alloc] initWithTitle:DLPhotoPickerLocalizedString(@"myAlbum",nil) style:UIBarButtonItemStylePlain target:self action:@selector(showAlbums:)];
}
...
#pragma mark - 打开相册列表
-(void)showAlbums:(UIBarButtonItem *)sender
{
        DLPhotoTableViewController *albumTableViewController = [[DLPhotoTableViewController alloc] init];
        albumTableViewController.showsLeftCancelButton = YES;
    albumTableViewController.navigationItem.title = DLPhotoPickerLocalizedString(@"Album list",nil);
...
//    [self presentViewController:albumTableViewController animated:true completion:^{
//
//    }];
